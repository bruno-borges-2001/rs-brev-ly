...
export default function useCreateLink() {
  return useMutation({
    ...
    mutationFn: (data: CreateLinkBody) => api.post<{ shortUrl: string }>('/link/create', data),
    onMutate: async (data: CreateLinkBody) => {
      queryClient.setQueryData<SavedLinksResponse | undefined>(['saved-links'], (old) => {
        ...
        return {
          ...old,
          links: [
            { id: "-1", url: data.url, shortUrl: data.shortUrl, accessCount: 0, createdAt: new Date().toISOString() },
            ...old.links
          ]
        }
      })
